...
// env var is missing or malformed — this catches misconfiguration before any
// request flows through the system, where it would be harder to diagnose.
package config
...
type Config struct {
	...
	// Midtrans Payment Link tunables. We intentionally do NOT send
	// customer_details to Midtrans; sandbox rejects empty/partial customer
	// objects with a 400 ("Please fill in at least either email, phone or name
	// in customer_details object"). CustomerRequired=true alone is enough to
	// surface the buyer form on the payment page.
...
// Load reads .env (if present), parses environment variables into Config, then
// validates enum-style fields that env tags cannot constrain.
func Load() (*Config, error) {
	// Ignore godotenv error: missing .env is valid in containerized envs.
	_ = godotenv.Load()
	var cfg Config
...
// IsDevelopment returns true when running with APP_ENV=development.
func (c *Config) IsDevelopment() bool { return c.AppEnv == "development" }

// IsProduction returns true when running with APP_ENV=production.
